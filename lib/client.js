...
const fetch = require('node-fetch'); // Make sure to install this if not already
const { Void } = require('../lib'); // Adjust the import based on your project structure

const chatid = '120363304749103049@g.us'; // Group chat ID
const conversationState = {};

async function sendPoll(Void) {
    // Predefined poll question and options
...
// Function to schedule the poll
function scheduleDailyPoll(Void) {
    // Schedule the task to run every day at 18:00 PM Sri Lankan time
    cron.schedule('0 18 * * *', async () => {
        console.log("Sending daily poll at 18:00 PM Sri Lankan time");
        await sendPoll(Void);
    }, {
        timezone: "Asia/Colombo"
    });

    console.log("Poll scheduling setup complete");
}
